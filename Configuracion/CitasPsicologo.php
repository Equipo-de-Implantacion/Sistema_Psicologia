...
// Consulta para paciente mayor de edad, modificada para mostrar todas las citas
...
INNER JOIN usuario AS us ON us.Id_Usuario = p.Id_Usuario WHERE ct.Status_Cita!= 'Suspendido' AND ct.Status_Cita!= 'Espera' AND ct.Status!= 'Cancelado' AND ct.Status!= 'Espera' AND cl.Status = 'Activo' AND p.Status = 'Activo' ";

// Consulta para paciente menor de edad, también modificada para mostrar todas las citas
...
INNER JOIN usuario AS us ON pm.Id_Usuario = us.Id_Usuario WHERE ct.Status_Cita!= 'Suspendido' AND ct.Status_Cita!= 'Espera' AND ct.Status!= 'Cancelado' AND ct.Status!= 'Espera' AND cl.Status = 'Activo' AND pm.Status = 'Activo' ";

$Declaracion1 = $Conexion->prepare($Consulta1);
if ($Declaracion1->execute() === false) {
    header("Location:../PHP/Citas_Psicologo.php?error=Error_Consulta1");
    exit();
}
...
if ($Declaracion2->execute() === false) {
    header("Location:../PHP/Citas_Psicologo.php?error=Error_Consulta2");
    exit();
}
