...
if (isset($_POST['accion_aceptar'])) {
    ...
    // Verificamos si hay alguna cita seleccionada
    if (isset($_POST['seleccionar']) && count($_POST['seleccionar']) > 0) {
        // Preparar y ejecutar la consulta para actualizar el estado de la cita a Aprobado y Status_Cita a Agendada
        foreach ($_POST['seleccionar'] as $Fila) {
            $datos = explode("|", $Fila);
            if (count($datos) == 4) {
                $idCita = $datos[1]; // ID de la cita
...
if (isset($_POST['accion_cancelar'])) {
    if (isset($_POST['seleccionar']) && count($_POST['seleccionar']) > 0) {
        ...
        // Actualizar el estado del calendario a Inactivo
